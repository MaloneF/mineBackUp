...
CheckUtil {
     ...
     * 校验用户唯一
     *
     * @param customerEntity        用户对象
     * @param operateType 操作类型（新增：1；修改：2）
     * @return 通用返回实体（非分页）
     */
    @SuppressWarnings("Duplicates")
    public static AjaxResult checkUserUnique(CustomerEntity customerEntity, int operateType) {
        // 定义sysUser
        SysUser sysUser = new SysUser();
        // 赋值用户id
        sysUser.setUserId(customerEntity.getUserId());
        // 赋值用户账号
        sysUser.setUserName(customerEntity.getUserName());
        // 赋值手机号
        sysUser.setPhonenumber(customerEntity.getPhonenumber());
        // 赋值邮箱
        sysUser.setEmail(customerEntity.getEmail());
        // 新增则校验登录帐号
        if (operateType == 1) {
            // 校验
